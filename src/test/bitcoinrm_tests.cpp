...
#include "test/bitcoinrm_cltv_multisig_data.h"
...
BOOST_FIXTURE_TEST_SUITE(bitcoinrm_tests, BasicTestingSetup)
...
BOOST_AUTO_TEST_CASE(zcash_header_compatible)
{
    ...
    bitcoinrm_header.nHeight = 10000000;
    // `CBlockHeader.nReserved[5:7]` should be placed in the last 8 bytes of `ZcashBlockHeader.hashReserved`.
    bitcoinrm_header.nReserved[5] = 1234;
    bitcoinrm_header.nReserved[6] = 0;
    bitcoinrm_header.nSolution = std::vector<unsigned char> {
        0x11, 0x22, 0x33
    };
    ...
    BOOST_CHECK_EQUAL(bitcoinrm_header.nHeight, (uint32_t)zcash_header.hashReserved.GetUint64(0));
    BOOST_CHECK_EQUAL(bitcoinrm_header.nReserved[5], (uint32_t)zcash_header.hashReserved.GetUint64(3));
    BOOST_CHECK_EQUAL(bitcoinrm_header.nSolution[2], zcash_header.nSolution[2]);
}
