...
UniValue generateBlocks(std::shared_ptr<CReserveScript> coinbaseScript, int nGenerate, uint64_t nMaxTries, bool keepScript)
{
    ...
    while (nHeight < nHeightEnd)
    {
        ...
        if (pblock->nHeight < (uint32_t)params.GetConsensus().BCRMHeight) {
            // Solve sha256d.
...
// NOTE: Unlike wallet RPC (which use BCRM values), mining RPCs follow GBT (BIP 22) in using satoshi amounts
...
//  In a coinbase transaction: 1st output to miner, 2nd output to founders address
//  But founders portion of the reward is not available for sharing among pool miners
//  Therefore, for coinbasevalue, return (block reward - founders reward + tx fees)
...
UniValue getblocksubsidy(const JSONRPCRequest& request)
{
  ...
  if (request.fHelp || request.params.size() > 1)
    throw std::runtime_error(
      ...
      "\"founders\": f, (numeric, Only if reward >= 100 coins) Currently " + std::to_string(FOUNDERSREWARD) + "%.\n"
      "}\n"
      "\nExamples:\n"
  ...
  // Founders reward only if subsidy is at least 100 coins
  if (nReward >= 100 * COIN && nHeight >= Params().GetConsensus().BCRMHeightRegular) {
    nFoundersReward = FOUNDERSREWARD * nReward / 100;
    nReward -= nFoundersReward;
  }

  UniValue result(UniValue::VOBJ);
  result.push_back(Pair("miner", nReward));
  result.push_back(Pair("founders", nFoundersReward));
  return result;
}
...
UniValue estimatesmartfee(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"feerate\" : x.x,     (numeric, optional) estimate fee-per-kilobyte (in BCRM)\n"
