...
const std::string strMessageMagic = "Bitcoin RM Signed Message:\n";
...
bool static IsBCRMHardForkEnabled(int nHeight, const Consensus::Params& params) {
    return nHeight >= params.BCRMHeight;
}

bool IsBCRMHardForkEnabled(const CBlockIndex* pindexPrev, const Consensus::Params& params) {
    if (pindexPrev == nullptr) {
        return false;
    }

    return IsBCRMHardForkEnabled(pindexPrev->nHeight, params);
}

bool IsBCRMHardForkEnabledForCurrentBlock(const Consensus::Params& params) {
    AssertLockHeld(cs_main);
    return IsBCRMHardForkEnabled(chainActive.Tip(), params);
}
...
bool ReadBlockFromDisk(CBlock& block, const CDiskBlockPos& pos, const Consensus::Params& consensusParams)
{
    ...
    bool postfork = block.nHeight >= (uint32_t)consensusParams.BCRMHeight;
...
{
    if (nHeight < consensusParams.BCRMHeightRegular) {
        // No block reward if it is the genesis block
        if (nHeight == 0)
            return 0;

        // Corresponds to total 21 million Bitcoin supply
        if (nHeight == 1)
            return 21000000 * COIN;

        // Development costs
        if (nHeight == 4 || nHeight == 8 || nHeight == 11 || nHeight == 14 || nHeight == 17)
            return 60000 * COIN;

        // Premine for Bitcoin RM Team
        return 50000 * COIN;
    }

    int halvings = (nHeight - consensusParams.BCRMHeightRegular) / consensusParams.nSubsidyHalvingInterval;
    // Force block reward to zero when right shift is undefined.
    if (halvings >= 64)
        return 0;

    // Block reward varies based on block position within each Interval
    int pos = (nHeight - consensusParams.BCRMHeightRegular) % consensusParams.nSubsidyHalvingInterval;
    int blockstep = 120000;
    int subsidy[12] = {20, 30, 45, 100, 200, 400, 900, 950, 700, 500, 150, 80};
    CAmount nSubsidy = subsidy[pos/blockstep] * COIN;
        
    // Subsidy is cut in half every nSubsidyHalvingInterval blocks
    nSubsidy >>= halvings;

    // No subsidy if it is less than 1 coin
    if (nSubsidy < 1 * COIN)
        return 0;
    else
        return nSubsidy;
...
bool IsInitialBlockDownload()
{
    ...
    if (chainActive.Tip()->nChainWork < UintToArith256(chainParams.GetConsensus().nMinimumChainWork))
        return true;
    if (fSkipHardforkIBD && chainActive.Tip()->nHeight + 1 >= (int)chainParams.GetConsensus().BCRMHeight)
        return false;
    int64_t target_time = fBCRMBootstrapping ? (int64_t)chainParams.GetConsensus().BitcoinPostforkTime : GetTime();
...
static unsigned int GetBlockScriptFlags(const CBlockIndex* pindex, const Consensus::Params& consensusparams) {
    ...
    if (IsBCRMHardForkEnabled(pindex->pprev, consensusparams)) {
...
bool CheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    if (block.nHeight < (uint32_t)consensusParams.BCRMHeight) {
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& params, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    // Check block height for blocks after BCRM fork.
    if (nHeight >= consensusParams.BCRMHeight && block.nHeight != (uint32_t)nHeight)
    ...
    // Check timestamp
    if (block.GetBlockTime() > nAdjustedTime + std::min(consensusParams.BCRMMaxFutureBlockTime, MAX_FUTURE_BLOCK_TIME))
...
static bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev)
{
    ...
    if (nHeight >= consensusParams.BCRMHeight && nHeight < consensusParams.BCRMHeightRegular && consensusParams.BCRMPremineEnforceWhitelist) {
        ...
        // During premine window: No Founder's reward, so only 1 transaction plus dummy SEGWIT transaction
        if (!(block.vtx[0]->vout.size() == 1 || (block.vtx[0]->vout.size() == 2 && block.vtx[0]->vout[1].nValue == 0.0)))
            return state.DoS(
                100, error("%s: only one coinbase output is allowed, height: %d", __func__, nHeight),
        ...
        if (nHeight > 0) {
          ...
          if (!valid)
            return state.DoS(100, error("%s: not in premine whitelist, height: %d", __func__, nHeight), REJECT_INVALID, "bad-premine-coinbase-scriptpubkey");
        }
    }

    // Validation for Founders Reward. Founders Reward only when subsidy is at least 100 coins
    if (nHeight >= consensusParams.BCRMHeightRegular) {
        CAmount nSubsidy = GetBlockSubsidy(nHeight, consensusParams);
        if (nSubsidy >= 100 * COIN) {
            bool found = false;
            static const double FOUNDERSREWARD = 5.00;  // In percentage


            BOOST_FOREACH(const CTxOut& outputvar, block.vtx[0]->vout) {
                if (outputvar.scriptPubKey == Params().GetFoundersRewardScript((uint32_t)nHeight)) {
                    if (outputvar.nValue == FOUNDERSREWARD * nSubsidy / 100) {
                        found = true;
                        break;
                    }
                }
            ...
            if (!found)
                return state.DoS(100, error("%s: founders reward missing, height: %d", __func__, nHeight), REJECT_INVALID, "cb-no-founders-reward");
        }
    }
